...
- **The repo under review — governed by peerreview's own convergence gate, never by trust.** Every run derives a fresh active charter from the current instruction and durable project sources, every round re-reviews the real diff and re-runs the verification gate, and a dismissed PEER finding is confirmed against the cited `file:line`. Auto-derivation removes a routine confirmation stop but never permits circular intent inferred from implementation; absent or conflicting durable intent fails closed. The preferred gate pairs Claude and Codex; OpenCode is the disclosed degraded fallback, with same-HOST review tertiary. Standard convergence requires active charter met + gates green + no substantive findings + explicit PEER `CONVERGED`, with a hard floor of 1 peer round; the tertiary path follows the skill's disclosed bounded HOST-native fallback bar.
